 ...
 */
package com.pheiffware.lib.demo.physics.testScenarios;

import com.pheiffware.lib.demo.physics.TestPhysicsScenario;
...
import com.pheiffware.lib.physics.entity.physicalEntity.physicalEntities.WallEntity;

/**
 *
 */
public class GeneralScenario2 extends TestPhysicsScenario
{
	public GeneralScenario2(double scenarioRuntime, int numSteps)
	{
	...
	public void setup(PhysicsSystem physicsSystem)
	{
		...
		SphereEntity circle2 = new SphereEntity(new Vec3D(0, 500, 0), new Vec3D(250, -80, 0), 25, 0.85f, 40);
		physicsSystem.addEntity(circle2);
		physicsSystem.addEntity(new SphereEntity(new Vec3D(200, 400, 0), new Vec3D(0, 0, 0), 5, 0.85f, 20));
		physicsSystem.addEntity(new SphereEntity(new Vec3D(250, 400, 0), new Vec3D(0, 0, 0), 5, 0.85f, 20));
		physicsSystem.addEntity(new SphereEntity(new Vec3D(300, 400, 0), new Vec3D(0, 0, 0), 5, 0.85f, 20));
		physicsSystem.addEntity(new SphereEntity(new Vec3D(350, 400, 0), new Vec3D(0, 0, 0), 5, 0.85f, 20));
		...
		physicsSystem.addEntity(new DirectionalGravityEntity(new Vec3D(600, 600, 0)));
	}
