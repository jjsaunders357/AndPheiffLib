...
/**
 * Sets up a background thread to manage updating the physics system. It can be queried for a cheap "render-copy" in a synchronized way.
 */
public abstract class PhysicsSystemManager
{
	...
	// This is set to make a request of the update thread. Without this, the physics processing thread may hog the monitor lock.
	...
	public void stop()
	{
		...
		{
			throw new RuntimeException("Join should never be interrupted.", exception);
		}
	}
			...
			while (alive)
			{
				...
				if (request)
				{
					...
					catch (InterruptedException exception)
					{
						...
						throw new RuntimeException("Illegal interrupt", exception);
					}
				}
				// long start = System.nanoTime();
				// while (System.nanoTime() - start < 1000 && request == true)
				// {
				// // Just wait. A sleep here is unacceptable as this seems to put the thread to sleep for at least 1ms.
				// }
			}
		}
	}
	...
	/**
	 * Called regularly to actually perform the update in the extending class. Is guaranteed to be synchronized.
	 */
	protected abstract void updateImplement(PhysicsSystem physicsSystem);

	/**
	 * Allows outside systems to get a "cheap" copy for rendering purposes. Give a list of all entities in a state serialized enough to be rendered.
	 */
